...
SharedFFN（NS 侧）和 SeqSharedFFN（序列侧）均使用 SwiGLU 激活函数替代标准 FFN 的 GELU / ReLU。
...
**SwiGLU：**
$$
\text{SwiGLU}(x) = W_{\text{out}} \cdot \left(\text{SiLU}(W_{\text{gate}} x) \odot W_{\text{value}} x\right)
$$

其中 $\odot$ 为逐元素乘，$\text{SiLU}(x) = x \cdot \sigma(x)$。

### 参数量

标准 FFN 使用 2 个 Linear（分别映射到 $d_{\text{hidden}}$ 和 $d_{\text{model}}$），SwiGLU 使用 3 个 Linear（gate、value、out）。

||标准 FFN|SwiGLU|
|--|-------|------|
|权重数|2 个 Linear|3 个 Linear|
|参数量|$2dd_h$|$3dd_h$（约 1.5×）|
