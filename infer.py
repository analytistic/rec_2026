...
_FALLBACK_MODEL_CFG = {
    ...
    'hidden_mult': 4,
    'embed_dropout_rate': 0.01,
    'seq_id_dropout_rate': 0.02,
...
def _batch_to_model_input(
    ...
    dow = device_batch.get('dow', torch.zeros(B, dtype=torch.long, device=device))
    weekend = device_batch.get('weekend', torch.zeros(B, dtype=torch.long, device=device))
...
def main() -> None:
    ...
    if ckpt_path is None:
        raise FileNotFoundError(
            ...
            f"The directory contains: {os.listdir(model_dir) if model_dir and os.path.exists(model_dir) else 'N/A'}. "
